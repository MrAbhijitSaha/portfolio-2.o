	...
	return (
		<div
			...
			<motion.div
				...
				className="relative flex h-auto w-full max-w-4xl items-center justify-between rounded-2xl border border-gray-200 bg-white/80 px-3 py-3 shadow-lg backdrop-blur-sm transition-all duration-300 hover:shadow-xl dark:border-gray-700 dark:bg-neutral-950 [@media(min-width:768px)]:px-6">
				{logoSrc && (
				...
				<nav className="hidden items-center space-x-4 [@media(min-width:640px)]:flex [@media(min-width:768px)]:space-x-6">
					{navItems.map((item, index) => (
						<Link
							key={index}
							href={item.href}
							className="group relative text-sm font-medium text-gray-700 transition-colors duration-200 hover:text-gray-900 dark:text-gray-200 dark:hover:text-white [@media(min-width:768px)]:text-base">
							{item.label}
				...
				<AnimatePresence>
					{mobileMenuOpen && (
						<motion.div
							...
							exit={{ opacity: 0, y: -10 }}
							transition={{ duration: 0.2 }}
							className="absolute top-full right-0 left-0 z-[999] mt-2 rounded-xl border border-gray-200 bg-white shadow-lg backdrop-blur-sm dark:border-gray-700 dark:bg-neutral-950 [@media(min-width:640px)]:hidden">
							<nav className="flex flex-col py-2">
								{navItems.map((item, index) => (
									<Link
										...
										className="px-4 py-3 text-sm font-medium text-gray-700 transition-colors duration-200 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-200 dark:hover:bg-slate-700 dark:hover:text-white">
										{item.label}
									</Link>
								))}
							</nav>
